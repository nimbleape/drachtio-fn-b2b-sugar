...
class CallManager extends Emitter {
  ...
  simring(anotherUriList, additionalCallOpts) {
    ...
    const simrings = this.uriList.map((uri) => this._attemptOne(uri, additionalCallOpts));
  ...
  addUri(uri, additionalCallOpts) {
    assert.ok(this.started, 'CallManager#addUri should not be called until simring has started');
    const p = this.intPromise = oneSuccess([this.intPromise, this._attemptOne(uri, additionalCallOpts)]);
    p
      .then((obj) => {
        if (p === this.intPromise) {
          this.finished = true;
  ...
  _attemptOne(uri, additionalCallOpts) {
    let timer = null;
    let uac, uas;

    // return a Promise that resolves to a {uac, uas} pair
    const mergedOpts = _.merge({}, this.callOpts, additionalCallOpts);
    this._logger.debug('CallManager#_attemptOne: merged call options: ' + JSON.stringify(mergedOpts));
    const p = this.srf.createUAC(uri, mergedOpts,
      {
